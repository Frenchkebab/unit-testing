...
contract BlockNumber {
    ...
    function gambleOnTenthBlockNumber() external payable {
        ...
        // require(address(this).balance >= 1 ether, "can't gamble with you"); // redundant check
        require(block.number > degenToBlockNumberBet[msg.sender] + MAX_BLOCKS_AHEAD, "wait for cooldown time");

        degenToBlockNumberBet[msg.sender] = block.number + MIN_BLOCKS_AHEAD;
    }

    function claimWinnings() external {
        require(block.number > MIN_BLOCKS_AHEAD + degenToBlockNumberBet[msg.sender], "too early");

        require(block.number <= MAX_BLOCKS_AHEAD + degenToBlockNumberBet[msg.sender], "too late");
